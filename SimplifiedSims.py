    ...
    for fieldid in pd.unique(stars.loc[:,'field_ID']):
        ...
        if not os.path.exists(os.path.join(field_file_loc,str(fieldid).zfill(2)+ext+'_generated_field')) or overwrite:
            ...
            if MP:
                ...
                print(allfieldstars.loc[allfieldstars['system_label']=='PL'].shape)
            ...
            print(finalfieldstars.shape,finalfieldstars.columns)
            finalfieldstars=addContams(finalfieldstars,BGstars,contam_prop=0.5)
...
def getBEBs(fieldstars,stars,bgstars,field_file_loc,num_quarts=8):
    ...
    while nbebs<len(fieldstars):
        ...
        tran_bebs_i['AB_Pmag']=tran_bebs_i['A_Pmag'].values - \
                               2.5*np.log10(1+np.power(2.512,tran_bebs_i['A_Pmag'].values-tran_bebs_i['Pmag'].values))
        -1*np.log(np.power(2.512,-1*tran_bebs_i['Pmag'].values) +\
                                         np.power(2.512,-1*tran_bebs_i['A_Pmag'].values))/np.log(2.5)
        tran_bebs_i['rms_hr']=getPmagRms(tran_bebs_i['AB_Pmag'],np.tile(np.nanmedian(fieldstars['Nscopes']),len(tran_bebs_i)))[0]
        tran_bebs_i['bsec']=abs(b_ecc_sec(tran_bebs_i['ecc'].values,tran_bebs_i['omega'].values,
                                    tran_bebs_i['sma'].values,tran_bebs_i['incl'].values,
                                    tran_bebs_i['Rs'].values)) #impact parameter at primary eclipse assuming Rb=0.5Ra
        #removing those ebs which do not transit:
        ...
        '''print(np.percentile(tran_bebs_i['A_Teff'],[5,50,95]),#OK
              ...
              np.percentile(tran_bebs_i['incl'],[5,50,95]),#OK
              np.percentile(tran_bebs_i['ecc'],[5,50,95]),#OK
              np.percentile(tran_bebs_i['sma'],[5,50,95]),#OK
              np.percentile(tran_bebs_i['omega'],[5,50,95]),#OK
              np.percentile(tran_bebs_i['Rs'],[5,50,95]),#OK
              np.percentile(tran_bebs_i['A_Rs'],[5,50,95]),#OK
              np.percentile(tran_bebs_i.loc[~np.isnan(tran_bebs_i['dilution']),'dilution'],[5,50,95]),#nan
              np.percentile(tran_bebs_i.loc[~np.isnan(tran_bebs_i['depthpri_ppm']),'depthpri_ppm'],[5,50,95]),#nan
              np.percentile(tran_bebs_i.loc[~np.isnan(tran_bebs_i['depthsec_ppm']),'depthsec_ppm'],[5,50,95]),#nan
              np.percentile(tran_bebs_i['rms_hr'],[5,50,95]),
              np.percentile(tran_bebs_i.loc[~np.isnan(tran_bebs_i['durpri']),'durpri'],[5,50,95]),#nan
              np.percentile(tran_bebs_i.loc[~np.isnan(tran_bebs_i['dursec']),'dursec'],[5,50,95]),#nan
              np.percentile(tran_bebs_i['P'],[5,50,95])
             )
        print(np.sum((tran_bebs_i['SNRpri'].values<4.0)&(tran_bebs_i['SNRsec'].values<4.0))," out of ",len(tran_bebs_i)," are low-SNR")'''
...
def getBPLs(fieldstars,stars,bgstars,field_file_loc,num_quarts=8):
...
if __name__=="__main__":
    ...
    if len(sys.argv)<8:
        ...
        args=sys.argv[:8]
    _=SimpleSim(hemisphere=args[1],N_fields=int(args[2]),ext=args[3],outfileloc=args[4],
